...
/**
 * Top navigation - Top top bar
 *
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'foundationpress_top_top_bar' ) ) {
	function foundationpress_top_top_bar() {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s top-top-bar">%3$s</ul>',
			'theme_location' => 'top-top-bar',
			'depth'          => 1,
			'fallback_cb'    => false,
			'walker'         => new Foundationpress_Top_Bar_Walker(),
		));
...
/**
 * Custom Header Nav for Desktop
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'custom_header_nav' ) ) {
	function custom_header_nav($menu) {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu_class'     => 'submenu',
			'theme_location' => $menu,
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-toggle>%3$s</ul>',
			'fallback_cb'    => false,
			'depth'					 => 2,
...
if ( ! function_exists( 'custom_footer_nav' ) ) {
	function custom_footer_nav($menu) {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
...
/**
 * Custom Header Nav for Mobile
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'custom_header_nav_mobile' ) ) {
	function custom_header_nav_mobile($menu) {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu_class'     => 'vertical nested menu',
			'theme_location' => $menu,
			'items_wrap'     => '<ul id="%1$s" class="%2$s">%3$s</ul>',
			'fallback_cb'    => false,
			'depth'					 => 1,
 ...
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'custom_submenu_nav' ) ) {
	function custom_submenu_nav($menu) {
		wp_nav_menu( array(
			'container'      => false,                    
			'menu_class'     => 'custom-submenu',
			'theme_location' => $menu,
			'items_wrap'     => '<ul id="%1$s" class="%2$s">%3$s</ul>',
			'fallback_cb'    => false,
			'depth'					 => 2,
...
/*
  Function borrowed from Pat Eason
  http://pateason.com/check-pagepost-wordpress-menu/
  Checks to see if current page is in the menu
*/
